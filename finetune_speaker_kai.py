...
def run(rank, n_gpus, hps):
  ...
  model_dir =pathlib.Path(hps.model_dir)
  checkpoint_g = model_dir.joinpath("G_latest.pth")
  checkpoint_d = model_dir.joinpath("D_latest.pth")
  if checkpoint_g.exists() and checkpoint_d.exists():
      print("Found existing checkpoints")
      _, _, _, iterationG = utils.load_checkpoint(checkpoint_g.resolve().__str__(), net_g, None)
      _, _, _, iterationD = utils.load_checkpoint(checkpoint_d.resolve().__str__(), net_d, None)
      global_step = iterationG
      epoch_str = global_step + 1
      print(f"Resuming from epoch {epoch_str}")
      # hps.train.eval_interval = 500
      # exit(0)
  else:
      # load existing model
